    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-sm">
      <div className="absolute inset-y-0 right-0 flex max-w-full pl-10">
        <div className="w-screen max-w-md border-l border-novex-border bg-novex-surface1 p-6 text-novex-text-primary shadow-2xl overflow-y-auto flex flex-col justify-between">
              ...
              <div className="rounded-xl border border-novex-border bg-novex-surface2/60 p-4">
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-xs text-novex-text-secondary">
                    Tipo: {item.kind === "RECURRING" ? "Recorrente" : item.kind === "INSTALLMENT_PLAN" ? "Parcelado" : "Avulso"}
                  </span>
              ...
              <div className="border-t border-novex-border pt-4">
                ...
                <div className="space-y-2 text-[11px]">
                  <div className="flex items-center justify-between text-novex-text-muted">
                    <span>Conta registrada no sistema</span>
          ...
          {/* Botão de Excluir Registro no rodapé da Auditoria */}
          <div className="pt-6 mt-6 border-t border-novex-border">
            <button
              onClick={handleDelete}
              className="w-full flex items-center justify-center gap-2 rounded-lg border border-red-500/40 bg-red-500/10 py-2.5 text-xs font-bold text-red-400 hover:bg-red-500/20 transition-all cursor-pointer"
            >
              <Trash2 className="h-4 w-4" />
              <span>Excluir / Apagar esta Conta</span>
            </button>
          </div>
        </div>
      </div>
